        ...
        <ul className="divide-y divide-gray-200">
          {Array.isArray(postData) &&
            postData.map((post, index) => (
              <li key={index}>
                <a
                  className="text-body1 whitespace-nowrap flex py-2"
                  href={`/posts/${post.id}`}
                >
                  <span className="font-bold text-gray-900 mr-3">
                    {index + 1}
                  </span>
                  <span className="text-black mr-1.5 break-all font-medium min-w-0 overflow-hidden overflow-ellipsis">
                    {post.title}
                  </span>
                  <div className="flex">
                    ...
                    <span className="mt-1 text-key text-[10px]">
                      {post.view_count}
                    </span>
                  </div>
                </a>
